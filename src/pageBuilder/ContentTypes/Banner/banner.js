...
const Banner = props => {
    ...
    if (contentMobile && !matchMedia('(min-width: 1024px)').matches) {
    ...
    useEffect(() => {
        ...
        if (backgroundType === 'video') {
            ...
            import(/* webpackChunkName: "jarallax" */ 'jarallax').then(
                ({ jarallax, jarallaxVideo }) => {
                    jarallaxVideo();
    ...
    const setBgImage = useCallback(() => {
        const resourceImage = resourceUrl(
            image,
            {
                type: 'image-wysiwyg',
                quality: 80,
    ...
    if (showButton !== 'never') {
        const resolvedLinksTo = resolveLinkProps(link).to;
        const resolvedLinksHref = resolveLinkProps(link).href;
        ...
        BannerButton = (
            <div className={buttonClass}>
                <Button
                    ...
                    onClick={() => {
                        if (resolvedLinksTo) {
                            history.push(resolvedLinksTo);
                        } else if (resolvedLinksHref) {
                            window.open(resolvedLinksHref, '_blank');
                        }
                    }}
                >
                    {buttonText}
    ...
    if (buttonType === 'link' && typeof link === 'string') {
        BannerButton = (
            <div className={buttonClass}>
                <LinkComponent
    ...
    if (typeof link === 'string' && !BannerButton) {
        BannerFragment = (
            <LinkComponent
 ...
 * @property {String} minHeightMobile CSS minimum height property for mobile
 ...
 * @property {String} contentMobile The HTML content to be rendered inside the banner content area for mobile width
...
Banner.propTypes = {
    ...
    mobileImage: string,
    mobileWidth: oneOfType([number, string]),
    mobileHeight: oneOfType([number, string]),
