...
const globalStoreConfig = new Map(
    Object.entries(globalThis?.UPWARD?.__STORE_CONFIG__ || {})
);

export const useStoreConfig = props => {
    const { fields } = props;
    const unresolvedFields = useMemo(
        () => fields.filter(field => !globalStoreConfig.has(field)),
        [fields]
    );
    const getStoreConfig = useMemo(
        () => getStoreConfigQuery(unresolvedFields),
        [unresolvedFields]
    );

    const [fetchStoreConfig, { data, called, loading, error }] = useLazyQuery(
        getStoreConfig,
        {
    ...
    const resultConfig = useMemo(() => {
        ...
        const resolvedFields = fields.filter(field =>
            globalStoreConfig.has(field)
        );
        ...
        resolvedFields.forEach(
            field => (result[field] = globalStoreConfig.get(field))
        );
