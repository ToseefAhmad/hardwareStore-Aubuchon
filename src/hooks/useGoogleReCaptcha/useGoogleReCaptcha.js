...
import defaultOperations from '@magento/peregrine/lib/hooks/useGoogleReCaptcha/googleReCaptchaConfig.gql';
...
export const useGoogleReCaptcha = props => {
    ...
    const [
        queryHandler,
        { data: configData, error: configError, loading: configLoading }
    ] = useLazyQuery(operations.getReCaptchaV3ConfigQuery, {
        fetchPolicy: 'cache-and-network'
    });

    useEffect(() => {
        isAppRendered && queryHandler();
    }, [isAppRendered, queryHandler]);

    if (!globalThis['recaptchaCallbacks']) {
        globalThis['recaptchaCallbacks'] = {};
    }
    const [apiIsReady, setApiIsReady] = useState(globalThis?.grecaptcha);
    ...
    const scriptUrl = (configData && new URL(GOOGLE_RECAPTCHA_URL)) || {};
