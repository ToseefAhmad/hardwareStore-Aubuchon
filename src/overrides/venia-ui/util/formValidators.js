...
export const validatePassword = value => {
    ...
    if (Object.values(count).filter(Boolean).length < 2) {
        const message = {
            ...
            defaultMessage:
                'A password must contain at least 2 of the following: lowercase, uppercase, digits, special characters.'
        };
        return message;
    }
...
export const ifNotEmptyHasLengthAtLeast = (value, length) => {
    return !value ? SUCCESS : hasLengthAtLeast(value, value, length);
};
...
export const isEmail = value => {
    ...
    return value?.trim().match(regexp) ? SUCCESS : FAILURE;
};

export const isPhone = (value, isRequired = true) => {
    const FAILURE = {
        id: 'validation.isPhoneValid',
        defaultMessage: 'Invalid phone number'
    };

    if (!isRequired && !value) return SUCCESS;

    return (!value && isRequired) || (value && value.length < 10)
        ? FAILURE
        : SUCCESS;
};
...
export const isYouTubeLink = value => {
    const FAILURE = {
        id: 'validation.isYouTubeLink',
        defaultMessage: 'Invalid YouTube link'
    };

    const regexp = /^(?:https?:\/\/)?(?:m\.|www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/;

    return !value || !value.match(regexp) ? FAILURE : undefined;
};
